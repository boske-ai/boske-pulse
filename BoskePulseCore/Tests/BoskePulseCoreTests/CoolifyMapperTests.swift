...
final class CoolifyMapperTests: XCTestCase {
    ...
    func testEndpointsFromDomainsUsesHTTPSRoot() {
        let endpoints = CoolifyMapper.endpoints(from: ["app.boske.dev"])
        XCTAssertEqual(endpoints.first?.url, "https://app.boske.dev/")
    ...
    func testMergedResourcesIncludesAliasCoolifyServers() {
        ...
        let coolifyServers = [
            CoolifyServer(uuid: "primary", name: "boske-app-01", ip: "167.233.70.227", isReachable: true, isUsable: true),
            CoolifyServer(uuid: "alias", name: "boska-app-01", ip: "167.233.70.227", isReachable: true, isUsable: true)
        ]

        let merged = CoolifyMapper.mergedResources(
            primaryUUID: "primary",
            serverName: "boske-app-01",
            publicIPv4: "167.233.70.227",
    ...
    func testManualContainersLinksEndpointHealthToAPI() {
        let tiles = CoolifyMapper.manualContainers(
            ...
            endpointChecks: [
                EndpointCheckResult(id: "app", label: "app.boske.dev", status: .ok, latencyMs: 42)
            ]
        )
        XCTAssertEqual(tiles.first?.health, .ok)
    ...
    func testMatchCoolifyServerByHetznerIP() {
        let servers = [
            CoolifyServer(uuid: "s", name: "boske-search-01", ip: "91.98.24.27", isReachable: true, isUsable: true)
        ]
        let match = CoolifyMapper.matchCoolifyServer(
            hetznerName: "boske-search-01",
            publicIPv4: "91.98.24.27",

...
**Repo:** `boske-ai/boske-pulse` · disk: `~/apps/canopystudio/apps/boske-pulse`
...
| 2 | Onboard `boske-search-01` + `boske-llm-01` into **Coolify** (one API for all containers) |
| 3 | **Tailscale** mesh — Mac reaches `10.0.0.0/16` and Coolify API without public exposure |
...
```
Mac (Boske Pulse)
  ├─ public health     → boske.dev, llm.boske.dev/healthz, search.boske.dev
  ├─ Coolify API       → via Tailscale (boske-website)
  ├─ Hetzner Cloud API → VM metrics (read-only token)
  ├─ private probes    → 10.0.0.2:5433 via Tailscale subnet route
  └─ alerts            → UserNotifications + Telegram bot

Coolify (after migration)
  ├─ boske-website     website, app API
  ├─ boske-data-01     licensing PG + data stack
  ├─ boske-search-01   searxng compose
  └─ boske-llm-01      llm-proxy compose
```

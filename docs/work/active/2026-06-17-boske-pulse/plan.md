...
tailscale up --advertise-tags=tag:boske-ops
```

### 1.2 Subnet routing on `boske-website`

```bash
tailscale up --advertise-routes=10.0.0.0/16 --accept-routes
```

Approve `10.0.0.0/16` in Tailscale admin.

### 1.3 Mac

- Tailscale installed; same tailnet
- ACL: Mac → `tag:boske-ops` ports 443, 5433, 8000

**Quality gate:** From Mac: `nc -zv 10.0.0.2 5433` succeeds; Coolify API on tailnet URL.
...
|--------|----------------------------|
| `boske-search-01` | `infra/docker/searxng/docker-compose.yml` |
| `boske-llm-01` | `infra/docker/llm-proxy/` |
...
| Check | Green | Red |
|-------|-------|-----|
| `https://boske.dev/` | HTTP 200 | else |
| `https://llm.boske.dev/healthz` | body contains `"ok":true` | else |
| `https://search.boske.dev/` | HTTP 200 | else |
